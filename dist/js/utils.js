"use strict";
const getAsset = (name) => assets[name];
/** handles user keyboard inputs */
const handleKeyInput = (_) => {
    if (lifeCount <= 0)
        return null;
    const { key, type } = _;
    if (type === "keyup")
...
const isBallCollidingWithBricks = (x, y, size, brickObj) => {
...
const showOutlines = (x, y, width, height) => {
    ctx.strokeStyle = "blue";
    ctx.rect(x, y, width, height);
    ctx.stroke();
};
/** draw image on canvas */
const drawImage = (sheet, sX, sY, sWidth, sHeight, x, y, width, height, config) => {
    const { show } = config || {};
    ctx?.beginPath();
    ...
    if (debugOutline) {
        if (show === "both") {
            ...
            return showOutlines(x, y, width, height);
    }
    else
        ctx.drawImage(sheet, sX, sY, sWidth, sHeight, x, y, width, height);
};
...
const makeMap = () => new Promise((res, rej) => {
    bricks = [];
    try {
...
const resetGame = () => {
    ...
    ready = true;
    render = false;
    lifeCount = initialLifeCount[level];
    resetPlayerPosition();
    resetBallPosition();
...
/** play selected level */
const playLevel = (levelSelected) => {
    const levelName = `level_${levelSelected}`;
    level = levelName;
    lifeCount = initialLifeCount[levelName];
    levelContainerEl.hide();
    resetGame();
    canvasEl.style.backgroundImage = `url(${backgrounds[levelName]})`;
    screenEl.hide();
};
